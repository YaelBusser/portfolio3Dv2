...
void main() {
  ...
  float time = mod(uTime * 0.001, 5.0); // Cycle time in seconds
  float factor = smoothstep(0.0, 1.0, fract(time / 5.0));
  ...
  vec3 color;
  if (time < 1.666) {
    color = interpolateColors(color1, color2, factor);
  } else if (time < 3.333) {
    color = interpolateColors(color2, color3, factor);
  } else {
